...
def get_addresses_by_client(client_id: int, db: Session = Depends(get_db)):
    def _execute():
...
def update_address(
    address_id: int, payload: AddressUpdate, db: Session = Depends(get_db)
):
    def _execute():
...
def deactivate_address(address_id: int, db: Session = Depends(get_db)):
    def _execute():
